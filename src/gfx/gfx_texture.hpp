...
struct GfxTexture {
  ...
  void create_from_rgba(uint8_t* data, uint16_t width, uint16_t height) noexcept;
...
void GfxTexture::create_from_rgba(uint8_t* data, uint16_t width, uint16_t height) noexcept
{
  GL_CHECK(glGenTextures(1, &this->id));
  GL_CHECK(glBindTexture(GL_TEXTURE_2D, this->id));
  DEBUG_ASSERT(glIsTexture(this->id) == GL_TRUE, assert_handler{});
  GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST));
  GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
  GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data));
  this->w = width;
  this->h = height;
}
